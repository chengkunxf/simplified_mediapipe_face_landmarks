...
# Nose = [64,48,115,220,45,51,3,196,197,419,248,281,275,440,344,278,294,326,2,97]
...
class mpFace:
    ...
    def __init__(self, width=640, height=480):
        ...
        self.faceMesh = self.mp.solutions.face_mesh.FaceMesh(False, 3, True, 0.8,
                                                             0.8)  # (staticFrame,number of faces,True for extra iris landmarks,trackingParameter,findingParameter)
        self.width = width
        self.height = height
    ...
    def faceLandmarksSimplified(self,
                                frame):  # essential face landmarks(left eyebrow,righteyebrow,left eye,right eye,inner lips,outer lips,face outline,left iris and right iris)
        frameRGB = self.cv2.cvtColor(frame, self.cv2.COLOR_BGR2RGB)
        results = self.faceMesh.process(frameRGB)
        # sequenced indexes of required landmarks from original landmarks
        points = [70, 63, 105, 66, 107, 55, 65, 52, 53, 46, 300, 293, 334, 296, 336, 285, 295, 282, 283, 276, 33, 246,
                  161, 160, 159, 158, 157, 173, 133, 155, 154, 153, 145, 144, 163, 7, 263, 466, 388, 387, 386, 385, 384,
                  398, 362, 382, 381, 380, 374, 373, 390, 249, 61, 185, 40, 39, 37, 0, 267, 269, 270, 409, 291, 375,
                  321, 405, 314, 17, 84, 181, 91, 146, 10, 338, 297, 332, 284, 251, 389, 356, 454, 323, 361, 288, 397,
                  365, 379, 378, 400, 377, 152, 148, 176, 149, 150, 136, 172, 58, 132, 93, 234, 127, 162, 21, 54, 103,
                  67, 109, 468, 469, 470, 471, 472, 473, 474, 475, 476, 477, 64, 48, 115, 220, 45, 51, 3, 196, 197, 419,
                  248, 281, 275, 440, 344, 278, 294, 326, 2, 97]
            ...
            for faceLandmarks in results.multi_face_landmarks:
                for lm, indx in zip(faceLandmarks.landmark, range(len(faceLandmarks.landmark))):
                    if indx in points:  # only collect required landmarks
                        myFaceLandmarksSimplified.append(
                            (int(lm.x * self.width), int(lm.y * self.height), int(lm.z * self.width)))
                    ...
                    for i, indx in zip(range(len(points)), myIndex):
                        myFaceLandmarksRearranged[indx] = myFaceLandmarksSimplified[
                            i]  # rearranging according to sequenced index
